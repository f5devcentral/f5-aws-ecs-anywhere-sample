...
output "EXTERNAL_SUBNET_ID" {
  value = aws_subnet.f5-volterra-external.id
}

output "WORKLOAD_SUBNET_ID" {
  value = aws_subnet.f5-volterra-internal.id
}

output "INTERNAL_SUBNET_ID" {
  value = aws_subnet.f5-volterra-workload.id
}
...
output "AWS_ENDPOINT_URL" {
  value = aws_vpc_endpoint.interface-s3.dns_entry
}